...
- 优化驾驶舱整体视觉风格：统一三栏工作台的圆角、阴影、边框、留白和按钮状态。
- 优化顶部连接状态显示，增加连接、重连、断开、错误的状态圆点。
- 优化左侧协作室与 Agents 卡片的折叠态和悬停层级，让高频“拉入协作室”操作更清晰。
- 优化中间聊天区的气泡、头像、系统提示、Markdown 内容和表格样式，提升长内容阅读体验。
- 优化右侧任务控制与任务流卡片的层级、状态标签和阶段详情可读性。
- 优化配置工作台的导航、工具栏、文件标签和编辑器视觉层级。
